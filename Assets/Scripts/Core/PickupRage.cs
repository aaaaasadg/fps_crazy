...
public class PickupRage : MonoBehaviour
{
    [Header("Movement")]
    ...
    [SerializeField] private float fireRateBonus = 1.0f; // +100% fire rate (2x)
    [SerializeField] private float damageBonus = 1.0f; // +100% damage (2x)
    ...
    private void ApplyRageBuff()
    {
        ...
        // Apply temporary buffs: +100% fire rate and +100% damage for 5 seconds
        playerStats.ApplyTemporaryBuff(StatType.FireRate, fireRateBonus, rageDuration);
        playerStats.ApplyTemporaryBuff(StatType.Damage, damageBonus, rageDuration);
    }
